...
public class CommandManager extends ZUtils implements CommandExecutor, TabCompleter {
	...
	private final ZVillagerPlugin main;
	private final List<VCommand> commands = new ArrayList<VCommand>();

	public CommandManager(ZVillagerPlugin template) {
		this.main = template;
	}
			...
			if (command.runAsync) {
				runAsync(() -> {
					...
					if (returnType == CommandType.SYNTAX_ERROR)
						message(sender, Message.COMMAND_SYNTAXE_ERROR, "%command%", command.getSyntaxe());
				});
				return CommandType.DEFAULT;
			}
	...
	public void sendHelp(String commandString, CommandSender sender) {
		commands.forEach(command -> {
			if (isValid(command, commandString)
				...
				message(sender, Message.COMMAND_SYNTAXE_HELP, "%syntaxe%", command.getSyntaxe(), "%description%",
